 ...
 */
export const CLAIM_PAGE_URL = 'https://jubjubapp.com/claim';

export function claimUrlFor(
  contentId: string | undefined | null,
  from: string = 'sdk',
): string | undefined {
  if (!contentId) return undefined;
  return `${CLAIM_PAGE_URL}?c=${encodeURIComponent(contentId)}&from=${encodeURIComponent(from)}`;
}
